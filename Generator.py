...
def get_graph_feature(x, k=20, idx=None, dim9=False):
    '''
    :param x: (b,c,n), points
    :param k: args, number of knn nearest points
    :param idx:
    :param dim9:
    :return: feature, (b, 2c, n)
    '''
    batch_size = x.size()[0]
    num_points = x.size()[2]
...
class Generator_backup(nn.Module):
    def __init__(self, args, dim=1024, in_dim=6):
        super(Generator_backup, self).__init__()
        self.dim = dim
        self.k = args.k
    ...
    def forward(self, pt, noise):
        ...
        x = self.conv4(x3)                                   # (batch_size, 128*2, num_points, k) -> (batch_size, 256, num_points, k)
        x4 = torch.max(x, dim=-1, keepdim=False)[0]         # (batch_size, 256, num_points, k) -> (batch_size, 256, num_points)

        pointfeat = x4                                      # (b, 1024, n)
        ...
        feat_r = pointfeat                                    # (batch_size, emb_dims)
        feat_r = torch.cat([feat_r, noise], 1)              # (batch_size, emb_dims + noise_dim)
        rotation, scale = self.rot(feat_r)                  # (batch_size, 3, 3), (batch_size)

        feat_d = pointfeat.view(-1, self.dim, 1).repeat(1, 1, N)   # (batch_size, emb_dims, 1) -> (batch_size, emb_dims, num_points)
        noise_d = noise.view(B, -1, 1).repeat(1, 1, N)

        feat_d = torch.cat((point_feat, feat_d, noise_d), 1) # (batch_size, 64+emb_dims+noise_dims, num_points)
...
class Generator_origin(nn.Module):
    def __init__(self, dim=1024,in_dim=3):
        ...
        self.rot = Generator_Rotation(self.dim)
        self.dis = Generator_Displacement(self.dim)
